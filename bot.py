...
ADMIN_CRYPTO_API, ADMIN_WALLET = range(10, 12)
...
async def reveal_cell(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if cell in mine_positions:
        ...
        # Update referral earnings (10% of loss)
        conn = sqlite3.connect(DB_NAME)
        c = conn.cursor()
        ...
        if ref_result and ref_result[0]:
            ...
            referral_bonus = bet * 0.1
            update_balance(referrer_id, referral_bonus)
            c.execute('UPDATE users SET referral_earnings = referral_earnings + ? WHERE user_id = ?',
...
async def admin_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    c.execute('SELECT SUM(bet) FROM games')
    total_bets = c.fetchone()[0] or 0
    
    c.execute('SELECT SUM(winnings) FROM games')
    total_winnings = c.fetchone()[0] or 0
    
    conn.close()
    
    keyboard = [
        [InlineKeyboardButton("💰 Добавить баланс пользователю", callback_data="admin_add_balance")],
        [InlineKeyboardButton("🔑 Управление CryptoBot API", callback_data="admin_crypto_api")],
    ...
    await query.edit_message_text(
        text=f"⚙️ АДМИН ПАНЕЛЬ\n\n"
             ...
             f"💸 Всего выплачено: ${total_winnings:.2f}",
        reply_markup=reply_markup
    )
...
async def admin_crypto_api(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    user_id = query.from_user.id
    ...
    await query.edit_message_text(
        text="🔑 Введите ваш CryptoBot API ключ:\n"
             "(Получить можно здесь: @CryptoBot)"
    )
    
    context.user_data['admin_action'] = 'set_crypto_api'
    return ADMIN_CRYPTO_API
...
async def admin_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("🔑 CryptoBot API", callback_data="admin_crypto_api")],
...
async def handle_admin_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == 'add_balance':
        try:
            ...
            conn = sqlite3.connect(DB_NAME)
            c = conn.cursor()
            c.execute('INSERT INTO deposits (user_id, amount) VALUES (?, ?)',
                     (target_user, amount))
            conn.commit()
            conn.close()
            
            await update.message.reply_text(
                f"✅ Пользователю {target_user} добавлено ${amount:.2f}"
            )
        except:
            await update.message.reply_text(
                "❌ Ошибка! Проверьте формат: USER_ID AMOUNT"
            )
    
    elif action == 'set_crypto_api':
        conn = sqlite3.connect(DB_NAME)
        c = conn.cursor()
        c.execute('INSERT OR REPLACE INTO admin_config (key, value) VALUES (?, ?)',
                 ('crypto_api', text))
        conn.commit()
        conn.close()
        
        await update.message.reply_text(
            "✅ CryptoBot API сохранен!"
        )
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(admin_crypto_api, pattern="^admin_crypto_api$"))
